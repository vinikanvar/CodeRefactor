...
/**
 * @version $Id: e992c64c4e676d0fe1197b8c042e810aa6ae1a27 $
 */
@Component
public class DefaultJerseyCookieSessionManager implements JerseyCookieSessionManager {
    ...
    public NewCookie[] getCookies(String username, String password, boolean remember) throws EncryptionException {
        ...
        int ageWhenRemember = 60 * (context.getRequest().isApiRequest() ? accountsSettings.getApiSessionDuration().getValue() : accountsSettings.getWebSessionDuration().getValue());
        NewCookie newUserCookie = new NewCookie("username", crypter.encrypt(username), "/", null, null, remember ? ageWhenRemember : -1, false);
        NewCookie newPassCookie = new NewCookie("password", crypter.encrypt(password), "/", null, null, remember ? ageWhenRemember : -1, false);

...
public class ListAddonTransformer implements AddonFieldTransformer {
    ...
    public Optional<AddonFieldValueWebObject> toWebView(EntityData<?> entityData, AddonFieldDefinition addonField, Object fieldValue) {
        ...
        for (final String value : values) {
            Optional<Map<String, Object>> definition = FluentIterable.from(listValues).filter(new Predicate<Map<String, Object>>() {

                public boolean apply(Map<String, Object> input) {
                    return input.containsKey("key") && input.get("key").equals(value.toString());
                }
            }).first();
            displayValues.add(definition.isPresent() && definition.get().containsKey("name") ? (String) definition.get().get("name") : value);
        }

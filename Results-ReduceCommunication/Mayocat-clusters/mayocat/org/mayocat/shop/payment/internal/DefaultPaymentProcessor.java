...
public class DefaultPaymentProcessor implements PaymentProcessor {
    ...
    public PaymentRequest requestPayment(Order order, Map<PaymentData, Object> data) throws PaymentException {
        ...
        final String gatewayId = data.containsKey(BasePaymentData.GATEWAY) ? (String) data.get(BasePaymentData.GATEWAY) : paymentsSettings.getGateways().getValue().get(0);
        ...
        if (!data.containsKey(BasePaymentData.IPN_URL)) {
            ...
            data.put(BasePaymentData.IPN_URL, urlHelper.getContextPlatformURL((webContext.getTenant() == null ? "marketplace/" : "") + "payment/" + order.getId() + "/acknowledgement/" + gatewayId));
        }
        try {
            ...
            if (CreditCardPaymentGateway.class.isAssignableFrom(gateway.getClass()) && !dataContainsCreditCard(data)) {
                return new PaymentRequest(PaymentStatus.NONE, RequiredAction.INTERNAL_FORM, ImmutableMap.<String, Object>of("creditCard", true));
            }
            ...
            if (gatewayResponse.isSuccessful()) {
                ...
                if (gatewayResponse.getOperation().getResult().equals(PaymentOperation.Result.CAPTURED)) {
                    ...
                    if (CreditCardPaymentGateway.class.isAssignableFrom(gateway.getClass())) {
                        ...
                        builder.withData(ImmutableMap.<String, Object>builder().putAll(gatewayResponse.getData()).put("creditCard", true).build());
                    }
                } else {
    ...
    private static boolean dataContainsCreditCard(Map<PaymentData, Object> data) {
        return data.containsKey(CreditCardPaymentData.CARD_NUMBER) && !Strings.isNullOrEmpty((String) data.get(CreditCardPaymentData.CARD_NUMBER));
    }
}

...
@Path("/customer")
@Controller
@RequestMapping("/ms0/customerrest/")
public class CustomerRESTWrapper {
    ...
    public @ResponseBody Integer getCustomer(@RequestParam int callerId, @CookieParam("sessionid") @RequestParam String sessionid, @PathParam("custid") @RequestParam String customerid) {
        CustomerRESTServer callerObj = (CustomerRESTServer) idObjMap.get(callerId);
    ...
    @Path("/byid/{custid}")
    @Produces("text/plain")
    @RequestMapping(method = RequestMethod.POST, value = "putcustomer")
    ...
    @RequestMapping(method = RequestMethod.POST, value = "customerrest")

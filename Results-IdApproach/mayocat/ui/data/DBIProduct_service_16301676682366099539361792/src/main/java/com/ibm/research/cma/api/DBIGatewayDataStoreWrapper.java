...
@Component
@Controller
@RequestMapping("/ms1/dbigatewaydatastore/")
public class DBIGatewayDataStoreWrapper {

    @RequestMapping(method = RequestMethod.POST, value = "storetenantdata")
    public @ResponseBody ResponseEntity<Void> storeTenantData(@RequestParam int callerId, @RequestParam int tenantDataId) {
        try {
            DBIGatewayDataStoreServer callerObj = (DBIGatewayDataStoreServer) idObjMap.get(callerId);
    ...
    @RequestMapping(method = RequestMethod.POST, value = "dbigatewaydatastore")

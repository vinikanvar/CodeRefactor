...
import org.mayocat.store.rdbms.dbi.*;
...
public class MoveEntityInListOperationWrapper<E extends Entity> {
    ...
    @RequestMapping(method = RequestMethod.POST, value = "moveentityinlistoperation")
    public  @ResponseBody int MoveEntityInListOperation(@RequestParam int entitiesId, @RequestParam String fromSlug, @RequestParam String toSlug, @RequestParam int relativePositionId) {
        MoveEntityInListOperationServer newServerObj = new MoveEntityInListOperationServer((List<E>) new Object(), fromSlug, toSlug, HasOrderedCollections.RelativePosition.getObject(relativePositionId));

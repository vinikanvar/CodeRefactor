...
import org.mayocat.cms.pages.store.jdbi.*;
...
@Controller
@RequestMapping("/ms0/dbipagestore/")
public class DBIPageStoreWrapper {
    ...
    public @ResponseBody ResponseEntity<Integer> create(@RequestParam int callerId, @Valid @RequestParam int pageId) {
        try {
            DBIPageStoreServer callerObj = (DBIPageStoreServer) idObjMap.get(callerId);
    ...
    @RequestMapping(method = RequestMethod.POST, value = "dbipagestore")

...
public interface AddonsDAO<T extends HasAddons> {
    ...
    @SqlQuery("SELECT * FROM addon WHERE entity_id = :entity.id")
    List<AddonGroup> findAddons(@BindBean("entity") T entity);

    @RegisterMapper(AddonGroupMapper.class)
    @SqlQuery("SELECT * FROM addon " + "WHERE    entity_id in ( <ids> )")
    List<AddonGroup> findAllAddonsForIds(@BindIn("ids") List<UUID> ids);

    @SqlUpdate("INSERT INTO addon " + "            (entity_id, " + "             source, " + "             addon_group, " + "             model," + "             value) " + "VALUES      (:entity.id, " + "             :addon.source, " + "             :addon.group, " + "             CAST (:addon.model AS json)," + "             CAST (:addon.value AS json)) ")
    void createAddonGroup(@BindBean("entity") T entity, @BindAddonGroup("addon") AddonGroup addon);

    @SqlUpdate("UPDATE addon " + "SET value = CAST (:addon.value AS json), " + "    model = CAST (:addon.model AS json) " + "WHERE entity_id = :entity.id " + "AND   source = :addon.source " + "AND   addon_group = :addon.group ")
    void updateAddonGroup(@BindBean("entity") T entity, @BindAddonGroup("addon") AddonGroup addon);

    @SqlUpdate("DELETE FROM addon " + "WHERE       addon.entity_id = :entity.id")
    Integer deleteAddons(@BindBean("entity") T entity);

    public int id = 0;

    public static AddonsDAO getObject(int id) {
        return null;
    }
}

...
import org.mayocat.shop.catalog.store.memory.*;
...
public class CollectionPositionedEntityWrapper {
    ...
    @RequestMapping(method = RequestMethod.POST, value = "collectionpositionedentity")
    public  @ResponseBody int CollectionPositionedEntity(@RequestParam int entityId, @RequestParam Integer position, @RequestParam int productsId) {
        CollectionPositionedEntityServer newServerObj = new CollectionPositionedEntityServer(Collection.getObject(entityId), position, (List<Product>) new Object());

 ...
 */
@Component
public class DefaultImageProcessorServer implements ImageProcessor {
    ...
    public RenderedImage scaleImage(Image image, Dimension dimension) {
        ResampleOp resampleOp = new ResampleOp((int) Math.round(dimension.getWidth()), (int) Math.round(dimension.getHeight()));
    ...
    public RenderedImage cropImage(Image image, Rectangle boundaries) {
        ...
        if (boundaries != null) {
            try {
                original = original.getSubimage(new Double(boundaries.getX()).intValue(), new Double(boundaries.getY()).intValue(), new Double(boundaries.getWidth()).intValue(), new Double(boundaries.getHeight()).intValue());

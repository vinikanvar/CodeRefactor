...
/**
 * @version $Id: 270b3e1822ea18d7b55ecb4b1ec4f105fc034424 $
 *
...
public class DefaultImageServiceTest {
    ...
    @Rule
    public final MockitoComponentMockingRule<ImageService> mocker = new MockitoComponentMockingRule(DefaultImageService.class, Arrays.asList(ImageProcessor.class));

    @Before
    public void setUp() throws Exception {
        this.fileManager = mocker.getInstance(FileManager.class);
        when(fileManager.resolvePermanentFilePath((Path) anyObject())).thenReturn(Paths.get(System.getProperty("java.io.tmpdir")));
    ...
    public void testGetFittingBoxWhenWidthIsLimiting() throws Exception {
        ...
        when(attachmentStore.findAndLoadById((UUID) anyObject())).thenReturn(attachment);
        Rectangle expected = new Rectangle(0, 50, 120, 100);
        Optional<Rectangle> box = this.mocker.getComponentUnderTest().getFittingRectangle(attachment, new Dimension(120, 100));
    ...
    @Ignore
    public // FIXME
    void testGetFittingBoxWhenHeightIsLimiting() throws Exception {
        // Width > Height
        ...
        Optional<Rectangle> box = this.mocker.getComponentUnderTest().getFittingRectangle(attachment, new Dimension(150, 150));
        Assert.assertEquals(expected, box.get());
    ...
    public void testGetFittingBoxWhenAspectRatioMatchesDimensions() throws Exception {
        ...
        attachment.setData(new AttachmentData(data));
        when(attachmentStore.findAndLoadById((UUID) anyObject())).thenReturn(attachment);
        Optional<Rectangle> box = this.mocker.getComponentUnderTest().getFittingRectangle(attachment, new Dimension(12, 10));

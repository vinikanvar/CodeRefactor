...
/**
 * @version $Id: ca4fdd5a0d0d399aa45157838f30f54d603ab170 $
 */
@Component("weight")
public class WeightStrategyPriceCalculator extends AbstractValueBasedStrategyPriceCalculator {
    ...
    protected BigDecimal getValue(Map<Purchasable, Long> items) {
        ...
        for (Purchasable purchasable : items.keySet()) {
            try {
                weight = weight.add(purchasable.getActualWeight().isPresent() ? purchasable.getActualWeight().get().multiply(BigDecimal.valueOf(items.get(purchasable))) : BigDecimal.ZERO);

...
public class CheckoutResource implements Resource {
    ...
    public Object checkout(final MultivaluedMap data) {
        ...
        if (checkoutRequest.getErrors().size() != 0) {
            ...
            return new WebView().template("checkout/form.html").with(WebView.Option.FALLBACK_ON_DEFAULT_THEME).data(bindings);
        }
        try {
    ...
    private Object generateCheckoutResponse(CheckoutResponse response) throws URISyntaxException {
        ...
        switch(paymentRequest.getNextAction()) {
            ...
            case INTERNAL_FORM:
                ...
                return new WebView().template("checkout/payment.html").with(WebView.Option.FALLBACK_ON_DEFAULT_THEME).data(bindings);
            ...
            default:
                ...
                } else if (paymentRequest.getStatus().equals(PaymentStatus.FAILED) || paymentRequest.getStatus().equals(PaymentStatus.REFUSED)) {
    ...
    public WebView returnFromPaymentService(@Context UriInfo uriInfo, @PathParam("order") String orderId) {
        ...
        return new WebView().template("checkout/success.html").with(WebView.Option.FALLBACK_ON_DEFAULT_THEME).data(bindings);
    }
    ...
    public WebView cancelFromPaymentService(@PathParam("orderId") UUID orderId) {
        try {
            ...
            return new WebView().template("checkout/exception.html").with(WebView.Option.FALLBACK_ON_DEFAULT_THEME).data(bindings);
        }
        Map<String, Object> bindings = new HashMap<>();
        return new WebView().template("checkout/cancelled.html").with(WebView.Option.FALLBACK_ON_DEFAULT_THEME).data(bindings);
    }

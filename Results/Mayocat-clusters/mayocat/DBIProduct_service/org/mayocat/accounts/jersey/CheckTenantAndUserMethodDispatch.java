    ...
    private class CheckValidTenantRequestDispatcher implements RequestDispatcher {
        ...
        public void dispatch(Object resource, HttpContext httpContext) {
            if (context.getTenant() == null) {
                throw new WebApplicationException(ErrorUtil.buildError(Response.Status.NOT_FOUND, StandardError.NOT_A_VALID_TENANT, "No valid tenant at this address"));
            }
        ...
        public void dispatch(Object resource, HttpContext httpContext) {
            ...
            if (user != null) {
                if (!this.checkAuthorization(user)) {
                    throw new WebApplicationException(Response.status(Response.Status.FORBIDDEN).entity(new Error(Response.Status.FORBIDDEN, StandardError.INSUFFICIENT_PRIVILEGES, "Insufficient privileges")).type(MediaType.APPLICATION_JSON).build());
                }
            } else {

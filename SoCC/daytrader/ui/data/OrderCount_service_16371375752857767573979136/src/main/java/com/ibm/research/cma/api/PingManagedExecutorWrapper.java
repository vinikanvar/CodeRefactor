...
@Controller
@RequestMapping("/ms0/pingmanagedexecutor/")
public class PingManagedExecutorWrapper {
    ...
    public @ResponseBody ResponseEntity<Void> doPost(@RequestParam int callerId, @RequestParam int reqId, @RequestParam int resId) {
        try {
            PingManagedExecutorServer callerObj = (PingManagedExecutorServer) idObjMap.get(callerId);
    ...
    @RequestMapping(method = RequestMethod.POST, value = "pingmanagedexecutor")
